...
restore_point = 27  # restore epoch
...
for iter_count in range(restore_point * saving_iter + 1, saving_iter * Max_iter+1):
    ...
    if iter_count % (saving_iter) == 0:
        ...
        for it in range(0, test_data_size, test_batch_size):
            ...
            cls[cls==22] = np_pred[cls==22]  # border를 정답으로 예측
            intersection = np.logical_and(cls, np_pred)
            union = np.logical_or(cls, np_pred)
            test_iou_score += np.sum(intersection) / np.sum(union) * len(img)
        ...
        test_iou_score = test_iou_score / test_data_size * 100
        ...
        for it in range(0, train_data_size, test_batch_size):
            ...
            cls[cls == 21] = np_pred[cls == 21] # border를 정답으로 예측
            intersection = np.logical_and(cls, np_pred)
            union = np.logical_or(cls, np_pred)
            train_iou_score += np.sum(intersection) / np.sum(union) * len(img)
        ...
        train_iou_score = train_iou_score / train_data_size * 100

        print(f"Train IoU : {train_iou_score} / Test IoU : {test_iou_score}")

        f = open('accuracy.txt', 'a+')
        f.write(f"epoch : {int((iter_count // saving_iter))}  /  Train IoU : {train_iou_score}  /  Test IoU : {test_iou_score}\n")
        f.close()